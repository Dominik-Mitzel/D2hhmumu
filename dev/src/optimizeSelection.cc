...
void optimizeSelectionInBins(TString cutNtracks="",TString q2Range="") { //do it for ntracks even and odd
  ...
  myFitter.fit_MC("deltaM>144.5&&deltaM<146.5"+nTrackSplitting+cutQ2Range,true,""); //fix MC signal shape from fit
  double FOM;
  ...
  //   for(int j=0;j<10;++j){
      ...
      cut_singleID = TString::Format("BDT>%f&&mu1_ProbNNmu>%f",BDTcuts[i],PIDcuts[j]);
      cutNormalizationChannel = cut_doubleID+nTrackSplitting+"&&D_DiMuon_Mass>675&&D_DiMuon_Mass<875";
      ...
      namePlot =  TString::Format("BDT%fPID%f",BDTcuts[i],PIDcuts[j])+cutNtracks+q2Range;
      
      myFitter.fit_Kpipipi_misID(cut_singleID+cutQ2Range,true,""); //no trck splitting here
      misIDBkg = myFitter.getMisIDbkgExp(cutNormalizationChannel,namePlot); 
      combBkg = myFitter.getCombBkg(cut_doubleID+nTrackSplitting+cutQ2Range,namePlot) * factor;

      misIDBKG_scaled = misIDBkg/33.14 * myEfficiency.getMCRelativeSigToNormMisIDEfficiency(TString::Format("BDT>%f",BDTcuts[i]),splitting,q2Range,"D_DiMuon_Mass>675&&D_DiMuon_Mass<875") 
	* myEfficiency.getMisIDFractionQ2Range(q2Range) * factor; //account for fact that only half of data is used, no PID for misID accounted for
 ...
 c->Divide(2,2);
  ...
  if(cutNtracks=="nTracksEven")c->Print("/work/mitzel/D2hhmumu/dev/D2KKmumu/img/selectionOptimisation/selection1D_nTracksEven_"+q2Range+".eps");
  if(cutNtracks=="nTracksOdd") c->Print("/work/mitzel/D2hhmumu/dev/D2KKmumu/img/selectionOptimisation/selection1D_nTracksOdd_"+q2Range+".eps");
...
void D2KKmumuData(){
  ...
  KK_Reader->createSubsample("/auto/data/mitzel/D2hhmumu/new/preselectedSamples/D2KKmumu_PreselectedSubsample1.root",100);
...
void D2pipimumuData(){
  ...
  pipi_Reader->createSubsample("/auto/data/mitzel/D2hhmumu/new/preselectedSamples/D2pipimumu_PreselectedSubsample.root",50);
...
void D2KpipipiData(){
  ...
  Kpipipi_Reader->createSubsample("/auto/data/mitzel/D2hhmumu/new/preselectedSamples/D2Kpipipi_PIDline_PreselectedSubsample1.root",10);
}
...
void D2KKpipiData(){
  ...
  KKpipi_Reader->createSubsample("/auto/data/mitzel/D2hhmumu/new/preselectedSamples/D2KKpipi_PIDline_PreselectedSubsample1.root",10);
}
...
void check_peakingBackground(TString kind,bool PIDCut=false) {
  ...
  TH2* h_mpiKpipi_mD = new TH2D ("h_mpiKpipi_vs_mD","h_mpiKpipi_vs_mD",20,1650,1950,20,1880,1950);
